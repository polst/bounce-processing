...
class BounceClassifier implements IBounceClassifier
{
    ...
    private $sourceRules = [
        ...
        self::TYPE_HARD => [
            ...
            'emaildoesntexist' => [
                ...
                '550 RCPT TO:.+? SMTP delivery not allowed',
                ...
                '550 <.+?@.+?>: invalid address',
                '553 Recipient .+? does not exist',
                '554 delivery error: dd This user doesn\'t have a .+?account',
                '553 Invalid user',
                '<.+?>: Sorry, no mailbox here by that name. \(#5.1.1\)',
                '550 Command RCPT User <.+?> not OK',
                'This is a permanent error. The following address\(es\) failed: .+? retry time not reached for any host after a long failure period',
            ...
            'inactive' => [
                ...
                '551 .+? is a deactivated mailbox',
                '550 RCPT TO:<.+?> Mailbox disabled',
        ...
        self::TYPE_SOFT => [
            ...
            'temporaryerror' => [
                ...
                'refused to talk to me: 421 .+? Service Unavailable',
                'refused to talk to me: 421 4.\d.\d Service not available',
                'refused to talk to me: 421 .+?SMTP - Too many connections',
                'refused to talk to me: 421 .+? Service refused, please try later',
                '450 4.2.2 .+?... temporary failure; please retry later',
                'Can\'t open mailbox for .+?. Temporary error',
                '421 .+?SMTP server temporari?ly not available',
            ...
            'blockedcontent' => [
                ...
                '553 5.7.1 .+?: Client host rejected: SPAM_CLIENT',
                'refused to talk to me: 550 Access denied...',
                '554 .+? bizsmtp [a-z\d]+ Connection refused from',
            ...
            'remoteconfigerror' => [
                ...
                '550 5.1.1 .+? Your IP has no Reverse DNS',
                '551 Server access .+? forbidden by invalid RDNS record of your mail server',
                ...
                '501 - Connection Refused: IP Address <.+?> - HELO/EHLO Invalid or Non-Existent Reverse DNS',
                'refused to talk to me: 421.+?Your host .+? has no valid Reverse DNS',
                ...
                'Name service error for name=.+? type=MX: Malformed or unexpected name server reply',
                '550-5.7.1 .+? Sorry, your helo has been denied',
                '550 5.7.1 <.+?>: Helo command rejected: Host not found',
                ...
                'said: 553 RP:RDN',
                '554 .+? Comcast requires that all mail servers must have a PTR record with a valid Reverse DNS entry',
                '501 5.7.1 .+? Sender IP must resolve',
                '421 4.\d.1 : \(DNS:NR\)',
                '421 4.1.2 .+?: Recipient address rejected: No reverse name for your IP address',
                '451 Blocked - Reverse DNS queries for your IP fail. You cannot send me mail',
                '452 4.1.0 Policy violation. Your host .+? has no valid Reverse DNS',
                '450 4.7.1 Client host rejected: cannot find your hostname',
                '450 4.7.1 Client host rejected: cannot find your reverse hostname',
                '450 4.7.1 <.+?>: Helo command rejected: Host not found',
                '521 .+? Service not available - no PTR record for',
            ],
    ...
    public function classifyBounce(string $emailHeaders, string $emailBody): array
    {
        ...
        if (
            preg_match('!^X-Autogenerated: Reply!mi', $emailHeaders, $ar)
            || preg_match('!^Auto-submitted: auto-generated$!mi', $emailHeaders, $ar)
            || (preg_match('!^X-AutoReply: yes!mi', $emailHeaders, $ar)
                && (preg_match('!^Auto-Submitted: auto-replied!mi', $emailHeaders, $asr)
                    || preg_match('!^X-AutoReply-From: (?<email>.+)!mi', $emailHeaders, $xAutoReplyFrom)))
        ) {
            $from = $xAutoReplyFrom ? $xAutoReplyFrom['email'] : $fromEmail;
            $ar = $asr ? "{$ar[0]} & {$asr[0]}" : $ar[0];
